...
| `/api/v1/*` returns 401 | `PG_ADMIN_TOKEN` not set or mismatch | Set `PG_ADMIN_TOKEN` in `.env`; Management API fails closed |fix(config): activate CORS origins + global deny env wiring)
